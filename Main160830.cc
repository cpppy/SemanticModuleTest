...
void* test(void* args) 
{
	...
	for(int i=0;ci<count;++i) 
	{
		...
		if(tdarr.size()>300)
		{
			// to retore the tdtime for every eg
			ofstream tdOutfile("tdArr.txt",ios::app);
			if(!tdOutfile)
			{
				cout<<"fail to open file 'tdArr.txt'"<<endl;
			}
			for(int i=0;i<tdarr.size();++i)
			{
				tdOutfile<<tdarr[i]<<endl;
			}
			tdOutfile.close();
			tdarr.clear();
		}
		...
		if(qOut.size()>200)
		{
			...
			if(!queryOutfile)
			{
				cout<<"fail to open file 'queryOutput.txt'"<<endl;
			}
...
//	std::cout<<YeWuConfig<<endl;
	...
	*/
	...
	if(argc==2)
	{
		ofstream pretdOutfile("tdArr.txt",ios::out);
		pretdOutfile.close();
		ofstream prequeryOutfile("queryOutput.txt",ios::out);
		prequeryOutfile.close();
		...
		timeval mainstarttime,mainendtime;
		...
		gettimeofday(&mainstarttime,0);                       //总时间计时开始-----------------------------------------------------
		for(int i=0;i<n;++i) {
			iarr[i] = i;
		...
		gettimeofday(&mainendtime,0);                       //总时间计时结束-----------------------------------------------------
		double maintd = 1000000*(mainendtime.tv_sec - mainstarttime.tv_sec) + mainendtime.tv_usec - mainstarttime.tv_usec;
		...
		*/
		...
		if(!tdOutfile)
		{
			cerr<<"fail to open file 'tdArr.txt'"<<endl;
			return -1;
		}
		for(int i=0;i<tdarr.size();++i)
		{
		...
		int cycle=100; //循环数，即为test函数中的count的第二个值
		double meantime=sum/tdarr.size();
		double flow=(120*cycle*n)/(maintd/1e+06);
		
		cout<<"************time count**************"<<endl;
		cout<<"lines : "<<n<<endl;
		cout<<"meantime = "<<meantime<<endl;
		cout<<"maintd = "<<maintd<<endl;
		cout<<"total quantity = "<<120*cycle*n<<endl;
		cout<<"mainflowpersec = "<<flow<<endl;
