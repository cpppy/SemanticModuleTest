...
int main()
{
	...
	for(int i=0;i<instrs.size();++i)
	{
		...
		count++;
		cout<<"query run : "<<count<<" times"<<endl;
	}	
	...
	for(int i=0;i<qIn.size();++i)
	{
		...
		qOut.push_back(timestr+"   "+orgStr[i]+"  "+sop);
...
bool MakeInParasProto(std::string sIn,std::string &sOut,vector<std::string> &strs)
{
	...
	paras.set_tid(strs[0]); //("10000001");
	paras.set_userid(strs[1]); //("200001231");
	// 1 NETWORKSTANDRD::NET_4G
	// 2 CHANNELTYPE::SHORTMESSAGE
	paras.set_networkstandard(*(strs[2].c_str())-'0'); //(1);
	paras.set_channeltype(*(strs[3].c_str())-'0'); //(2);
	paras.set_text(strs[4]); //(sIn);
	//paras.set_preprocesstype(7);
	paras.set_actiontype(strs[5]);  //("normal");
